...
local fakeNotif = { -- stuff thats playable in the fake sound option, uses special furfag.de page for youtube player embed and volume
        {
                name = "Discord Call",
...
local menu = MenuV:CreateMenu(" ", 'Developed By Tony', 'topright', 102, 0, 204, 'size-150', 'template1', 'menuv',
    'mainmenu')
	local onlineplayers = MenuV:CreateMenu(" ", 'Online Players', 'topright', 102, 0, 204, 'size-150', 'template1',
    'menuv', 'onlineplayers')
...
local playeroptions = MenuV:CreateMenu(" ", 'Player Related Options', 'topright', 102, 0, 204, 'size-150',
'template1', 'menuv', 'playerrelatedoption')
...
local playeroptions_custom_customize = MenuV:CreateMenu(" ", 'Ped Customization Menu', 'topright', 102, 0, 204, 'size-150',
'template1', 'menuv', 'playerrelatedoption_ped2')
...
local vehicleoptions_vehicleop_mod = MenuV:CreateMenu(" ", 'Mod Menu', 'topright', 102, 0, 204, 'size-150',
'template1', 'menuv', 'vehmodmenu')
local vehicleoptions_vehicleop_mod_colormenu = MenuV:CreateMenu(" ", 'Color Menu', 'topright', 102, 0, 204,
    'size-150', 'template1', 'menuv', 'vehmodmenucolor')
local vehicleoptions_vehicleop_mod_colors = MenuV:CreateMenu(" ", 'Classic Colors', 'topright', 102, 0, 204,
    'size-150', 'template1', 'menuv', 'vehmodmenuclasiccol')
local vehicleoptions_vehicleop_mod_tyresmoke = MenuV:CreateMenu(" ", 'Tyre Smoke Colors', 'topright', 255,
    0, 0, 'size-150', 'template1', 'menuv', 'vehmodmenutyrsmok')
local vehicleoptions_vehicleop_mod_metal = MenuV:CreateMenu(" ", 'Metal Colors', 'topright', 102, 0, 204,
    'size-150', 'template1', 'menuv', 'vehmodmenumetalcol')
local vehicleoptions_vehicleop_mod_matte = MenuV:CreateMenu(" ", 'Matte Colors', 'topright', 102, 0, 204,
    'size-150', 'template1', 'menuv', 'vehmodmenumattecol')
local vehicleoptions_vehicleop_mod_neon = MenuV:CreateMenu(" ", 'Neon Options', 'topright', 102, 0, 204,
    'size-150', 'template1', 'menuv', 'vehmodmenuneon')
...
local miscsettings = MenuV:CreateMenu(" ", 'Misc Settings', 'topright', 102, 0, 204, 'size-150',
'template1', 'menuv', 'miscsett')
local rockstar = MenuV:CreateMenu(" ", 'Rockstar Editor', 'topright', 102, 0, 204, 'size-150', 'template1', 'menuv', 'rockstar')

local trollmenu = MenuV:CreateMenu(" ", 'Troll Menu', 'topright', 102, 0, 204, 'size-150',
'template1', 'menuv', 'trollmenu')
...
menu.data.ClearItems(menu)
onlineplayers.data.ClearItems(onlineplayers)
onlineplayers_each.data.ClearItems(onlineplayers_each)
playeroptions.data.ClearItems(playeroptions)
playeroptions2.data.ClearItems(playeroptions2)
playeroptions_custom.data.ClearItems(playeroptions_custom)
playeroptions_custom_customize.data.ClearItems(playeroptions_custom_customize)
vehicleoptions.data.ClearItems(vehicleoptions)
vehicleoptions_vehiclespawn.data.ClearItems(vehicleoptions_vehiclespawn)
vehicleoptions_vehicleop.data.ClearItems(vehicleoptions_vehicleop)
vehicleoptions_vehicleop_mod.data.ClearItems(vehicleoptions_vehicleop_mod)
vehicleoptions_vehicleop_mod_colormenu.data.ClearItems(vehicleoptions_vehicleop_mod_colormenu)
vehicleoptions_vehicleop_mod_colors.data.ClearItems(vehicleoptions_vehicleop_mod_colors)
vehicleoptions_vehicleop_mod_tyresmoke.data.ClearItems(vehicleoptions_vehicleop_mod_tyresmoke)
vehicleoptions_vehicleop_mod_metal.data.ClearItems(vehicleoptions_vehicleop_mod_metal)
vehicleoptions_vehicleop_mod_matte.data.ClearItems(vehicleoptions_vehicleop_mod_matte)
vehicleoptions_vehicleop_mod_neon.data.ClearItems(vehicleoptions_vehicleop_mod_neon)
for k,v in pairs(vehicleslist) do
	spawnermenu[k].data.ClearItems(spawnermenu[k])
end
miscsettings.data.ClearItems(miscsettings)
rockstar.data.ClearItems(rockstar)
trollmenu.data.ClearItems(trollmenu)
		...
		ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(plyList)
			...
			for k, v in pairs(plyList) do
				...
				onlineplayers:AddButton({
					...
					false,
					select = function(i)
						local ply = {source = v.source, name = v.name}
						OpenPlayersMenu(ply)
					end
				})
			end
		...
		select = function(i)
			ESX.TriggerServerCallback('ts-adminmenu:server:GetJobs', function(jobs)
				...
				for k,v in pairs(jobs) do
					...
					count = count + 1
					table.insert(myMenu, {id = count, header = v.label..' - '..v.name, txt = 'Grades = '..grade - 1, params = {event = 'ts-adminmenu:client:setgrade', isServer = false, args = {job = v.name, max = grade}}})
				end
	...
	if allowed then
		local SelectItems = function()
			...
			for k,v in pairs(Items) do
				table.insert(myMenu, {id = count , header = v.label, txt = 'Give '..v.name, params = {event = 'ts-adminmenu:client:GiveItem', isServer = false, args = {item = v.name,plyid = selectedPlayer}}})
				count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
		end
	...
	if allowed then
		local SelectItems = function()
			...
			if keyboard ~= nil then
				...
				local data = {item = keyboard[1].input, plyid = selectedPlayer}
		...
		RegisterCommand('gi', function(source,args,raw)
			...
			if keyboard ~= nil then
				...
				local data = {item = keyboard[2].input, plyid = keyboard[1].input}
				TriggerEvent('ts-adminmenu:client:GiveItem', data)
	...
	local onlineplayers_each_givem = onlineplayers_each:AddButton({
		...
		select = function(i)
			...
			if keyboard ~= nil then
				...
				TriggerServerEvent('ts-adminmenu:server:GiveAccMoney', selectedPlayer,keyboard[1].input, keyboard[2].input )
			end
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	...
	local onlineplayers_each_removem = onlineplayers_each:AddButton({
		...
		select = function(i)
			...
			if keyboard ~= nil then
				if keyboard[1].input == nil or keyboard[2].input == nil then return end
				TriggerServerEvent('ts-adminmenu:server:RemoveAccMoney', selectedPlayer,keyboard[1].input, keyboard[2].input )
			end
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
		...
		local maxTexture_head = GetNumberOfPedTextureVariations(PlayerPedId(), 0, 0)
			local drawableList_head = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Head", min = 0, max = maxVariation_head, value = currentDrawable_head, saveOnUpdate = true })
			...
			if maxTexture_head > 0 then
				local drawableList_head2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Head Texture", min = 0, max = maxTexture_head, value = currentTexture_head, saveOnUpdate = true })
		...
		local maxTexture_Beard= GetNumberOfPedTextureVariations(PlayerPedId(), 1, 1)
			local drawableList_beard = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Beard", min = 0, max = maxVariation_Beard, value = currentDrawable_Beard, saveOnUpdate = true })
		...
		if maxTexture_Beard > 0 then
			local drawableList_beard2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Beard Texture", min = 0, max = maxTexture_Beard, value = currentTexture_Beard, saveOnUpdate = true })
		...
		local maxTexture_Hair = GetNumberOfPedTextureVariations(PlayerPedId(), 2, 2)
			local drawableList_Hair = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Hair", min = 0, max = maxVariation_Hair, value = currentDrawable_Hair, saveOnUpdate = true })
		...
		if maxTexture_Hair > 0 then
			local drawableList_Hair2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Hair Texture", min = 0, max = maxTexture_Hair, value = currentTexture_Hair, saveOnUpdate = true })
		...
		local maxTexture_Torso = GetNumberOfPedTextureVariations(PlayerPedId(), 3, 3)
			local drawableList_Torso = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Torso", min = 0, max = maxVariation_Torso, value = currentDrawable_Torso, saveOnUpdate = true })
		...
		if maxTexture_Torso > 0 then
			local drawableList_Torsot = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Torso Texture", min = 0, max = maxTexture_Torso, value = currentTexture_Torso, saveOnUpdate = true })
		...
		local maxTexture_Legs = GetNumberOfPedTextureVariations(PlayerPedId(), 4, 4)
			local drawableList_Legs = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Legs", min = 0, max = maxVariation_Legs, value = currentDrawable_Legs, saveOnUpdate = true })
		...
		if maxTexture_Legs > 0 then
			local drawableList_Legs2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Legs Texture", min = 0, max = maxTexture_Legs, value = currentTexture_Legs, saveOnUpdate = true })
		...
		local maxTexture_Hands = GetNumberOfPedTextureVariations(PlayerPedId(), 5, 5)
			local drawableList_Hands = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Hands", min = 0, max = maxVariation_Hands, value = currentDrawable_Hands, saveOnUpdate = true })
		...
		if maxTexture_Hands > 0 then
			local drawableList_Hands2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Hands Texture", min = 0, max = maxTexture_Hands, value = currentTexture_Hands, saveOnUpdate = true })
		...
		local maxTexture_Foot = GetNumberOfPedTextureVariations(PlayerPedId(), 6, 6)
			local drawableList_Foot = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Foot", min = 0, max = maxVariation_Foot, value = currentDrawable_Foot, saveOnUpdate = true })
		...
		if maxTexture_Foot > 0 then
			local drawableList_Foot2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Foot Texture", min = 0, max = maxTexture_Foot, value = currentTexture_Foot, saveOnUpdate = true })
		...
		local maxTexture_Misc = GetNumberOfPedTextureVariations(PlayerPedId(), 7, 7)
			local drawableList_Misc = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Misc", min = 0, max = maxVariation_Misc, value = currentDrawable_Misc, saveOnUpdate = true })
		...
		if maxTexture_Misc > 0 then
			local drawableList_Misc2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Misc Texture", min = 0, max = maxTexture_Misc, value = currentTexture_Misc, saveOnUpdate = true })
		...
		local maxTexture_Accs1 = GetNumberOfPedTextureVariations(PlayerPedId(), 8, 8)
			local drawableList_Accs1 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Accessory 1", min = 0, max = maxVariation_Accs1, value = currentDrawable_Accs1, saveOnUpdate = true })
		...
		if maxTexture_Accs1 > 0 then
			local drawableList_Accs1t = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Accessory 1 Texture", min = 0, max = maxTexture_Accs1, value = currentTexture_Accs1, saveOnUpdate = true })
		...
		local maxTexture_Accs2 = GetNumberOfPedTextureVariations(PlayerPedId(), 9, 9)
			local drawableList_Accs2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Accessory 2", min = 0, max = maxVariation_Accs2, value = currentDrawable_Accs2, saveOnUpdate = true })
		...
		if maxTexture_Accs2 > 0 then
			local drawableList_Accs2t = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Accessory 2 Texture", min = 0, max = maxTexture_Accs2, value = currentTexture_Accs2, saveOnUpdate = true })
		...
		local maxTexture_Decals = GetNumberOfPedTextureVariations(PlayerPedId(), 10, 10)
			local drawableList_Decals = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Decals", min = 0, max = maxVariation_Decals, value = currentDrawable_Decals, saveOnUpdate = true })
		...
		if maxTexture_Decals > 0 then
			local drawableList_Decals2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Decals Texture", min = 0, max = maxTexture_Decals, value = currentTexture_Decals, saveOnUpdate = true })
		...
		local maxTexture_Torso2 = GetNumberOfPedTextureVariations(PlayerPedId(), 11, 11)
			local drawableList_Torso2 = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Torso 2", min = 0, max = maxVariation_Torso2, value = currentDrawable_Torso2, saveOnUpdate = true })
		...
		if maxTexture_Torso2 > 0 then
			local drawableList_Torso2t = playeroptions_custom_customize:AddRange({ icon = '👕', label = "Torso 2 Texture", min = 0, max = maxTexture_Torso2, value = currentTexture_Torso2, saveOnUpdate = true })
...
CreateThread(function()
	...
	for k,v in pairs(vehicleslist) do
		...
		spawnermenu[k]:AddButton({ icon = '', label = j, value = 'adder', description = 'Spawn '..j, select = function(i)
			if cooldown then ESX.ShowNotification("Please Wait sometime before spawning again") return end
			cooldown = true
...
local neoncolor =  vehicleoptions_vehicleop_mod_neon:AddSlider({ icon = '', label = 'Neon Color', value = 'demo', values = {
    { label = 'White', value = {222,222,255}, description = 'Demo Item 1' },
    { label = 'Blue', value = {2,21,255}, description = 'Demo Item 2' },
    { label = 'Electric Blue', value = {3,83,255}, description = 'Demo Item 3' },
    { label = 'Mint Green', value = {0,222,140}, description = 'Demo Item 4' },
	{ label = 'Lime Green', value = {94,222,1}, description = 'Demo Item 4' },
	{ label = 'Yellow', value = {255,255,0}, description = 'Demo Item 4' },
	{ label = 'Golden Shower', value = {255,150,0}, description = 'Demo Item 4' },
	{ label = 'Orange', value = {255,62,0}, description = 'Demo Item 4' },
	{ label = 'Red', value = {255,1,1}, description = 'Demo Item 4' },
	{ label = 'Pony Pink', value = {255,50,100}, description = 'Demo Item 4' },
	{ label = 'Hot Pink', value = {255,5,190}, description = 'Demo Item 4' },
	{ label = 'Purple', value = {35,1,255}, description = 'Demo Item 4' },
	{ label = 'Blacklight', value = {15,3,255}, description = 'Demo Item 4' },
}})
            ...
            for k, v in pairs(vehmods) do
                if v.modType and type(v.modType) == 'number' then
					if v.modType == 17 then
						modbuttons[k] = vehicleoptions_vehicleop_mod:AddCheckbox({ icon = '', label = v.label, value = 'n'})
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local tonydoor = {}
local vehicleoptions_vehicleop_toggledoor = vehicleoptions_vehicleop:AddSlider({ icon = '🚗', label = 'Toggle Doors', value = 0, values = {
    { label = 'LFront Door', value = 0, description = '' },
    { label = 'RFront Door', value = 1, description = '' },
    { label = 'LRear Door', value = 2, description = '' },
    { label = 'RRear Door', value = 3, description = '' },
	{ label = 'Hood', value = 4, description = '' },
...
local vehicleoptions_vehicleop_torque = vehicleoptions_vehicleop:AddRange({ icon = '🚀', label = 'Torque Multiplier', min = 0, max = 10, value = 0, saveOnUpdate = true })
...
vehicleoptions_vehicleop_torqueen:On('uncheck', function(item)
	...
	local ped = PlayerPedId()
	local veh = GetVehiclePedIsIn(ped, false)
	SetVehicleCheatPowerIncrease(veh, 1)
...
local vehicleoptions_vehicleop_enginem_R = vehicleoptions_vehicleop:AddRange({ icon = '🚀', label = 'Engine Multiplier', min = 0, max = 10, value = 0, saveOnUpdate = true })
...
vehicleoptions_vehicleop_enginem:On('uncheck', function(item)
	...
	local ped = PlayerPedId()
	local veh = GetVehiclePedIsIn(ped, false)
	ModifyVehicleTopSpeed(veh, orgspeed)
	orgspeed = nil
	...
	if not engineinterval then
		local speed = 1.0
			...
			if engineenabled then
				ModifyVehicleTopSpeed(veh, orgspeed * speed)
			else
				ModifyVehicleTopSpeed(veh, orgspeed)
			end
		end
...
vehicleoptions_vehicleop_torque:On('change', function(item, newValue, oldValue)
	...
	if not torqueinterval then
		...
		torqueinterval = SetInterval(function()
			if veh ~= 0 then
			if torqueenabled then
				SetVehicleCheatPowerIncrease(veh, speed)
			else
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local vehicleoptions_vehicleop_flip = vehicleoptions_vehicleop:AddButton({ icon = '🚗', label = 'Flip Vehicle', value = 'n', description = 'Set Vehicle Properly', select = function(i)
	local ped = PlayerPedId()
	local veh = GetVehiclePedIsIn(ped, false)
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local vehicleoptions_vehicleop_delete = vehicleoptions_vehicleop:AddButton({ icon = '🚗', label = 'Delete Vehicle', value = 'n', description = 'Delete Vehicle you are in', select = function(i)
...
local misc_teleportcoords =  miscsettings:AddButton({
	...
	select = function(i)
		...
		if keyboard ~= nil then
			...
			SetPedCoordsKeepVehicle(PlayerPedId(), vec3(tonumber(keyboard[1].input),tonumber(keyboard[2].input), tonumber(keyboard[3].input)))
		end
	end
})
end
...
local misc_showcoords =  miscsettings:AddCheckbox({ icon = '📲', label = 'Show Coords', value = 'n' })
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local misc_thermalvision =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Thermal Vision', value = 'n' })
...
local misc_joiunq =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Join/Quit Notifications', value = 'n' })
...
RegisterNetEvent('esx:onPlayerDeath', function(data)
	...
	if not killed then
		...
		TriggerServerEvent('ts-adminmenu:server:playerDied', {player_id = GetPlayerServerId(PlayerId()), player_2_id = nil,death_reason = DeathReason})
	else
		if ClientFunc.IsMelee(DeathCauseHash) then
			DeathReason = "Murdered"
		elseif ClientFunc.IsTorch(DeathCauseHash) then
		...
		TriggerServerEvent('ts-adminmenu:server:playerDied', {player_id = GetPlayerServerId(PlayerId()), player_2_id = info.killerServerId,death_reason = DeathReason})
	end
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local misc_kill =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Kill Notifications', value = 'n' })
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local misc_nightvision =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Night Vision', value = 'n' })
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
		...
		local misc_playerblips =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Player Blips', value = 'n' })
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
		...
		local plynameint = SetInterval(function()
			...
			for k,v in pairs(ply) do
				...
				if dist < 250 then
					...
					plynamet[k] = CreateFakeMpGamerTag(GetPlayerPed(v), GetPlayerName(v)..': '..GetPlayerServerId(v), false,false, "", 0)
		...
		local misc_playername =  miscsettings:AddCheckbox({ icon = '🔍', label = 'Player Names', value = 'n' })
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
local misc_timecycles = miscsettings:AddSlider({ icon = '🎥', label = 'Timecycle', value = 'cinema', values = Timecycles})
local misc_timecycles_intensity = miscsettings:AddRange({ icon = '🎥', label = 'Timecycle Intensity', min = 0, max = 10, value = 0, saveOnUpdate = true })
...
misc_timecycles:On('change', function(item, value)
	SetTimecycleModifier(item.data.Values[value].Value) end)
end
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
		local trollmenu_fart = trollmenu:AddSlider({ icon = '🎥', label = 'Fart', value = 'bbfart', values = {
			{ label = 'Bass Boosted Fart', value = 'bbfart', description = 'Play Bass Boosted Fart' },
    		{ label = 'Common Fart', value = 'cmfart', description = 'Play Common Fart' },
    		{ label = 'Powerful Fart', value = 'pwrfart', description = 'Play Powerful Fart' },
    		{ label = 'Smelly Fart', value = 'smfart', description = 'Play Smelly Fart' }
		}})
		...
		local trollmenu_fake =  trollmenu:AddButton({
			...
			select = function(i)
				...
				for k,v in ipairs(fakeNotif) do
					local t = count - 1
					table.insert(myMenu, {id = count , header = v.name, txt = 'Play '..v.name, params = {event = 'ts-adminmenu:client:FakeCallply', isServer = false, args = {type = t}}})
					count = count + 1
				end
				exports['zf_context']:openMenu(myMenu)
			end
...
ESX.TriggerServerCallback("ts-adminmenu:server:IsAllowed", function(allowed)
	if allowed then
		local slider = rockstar:AddSlider({ icon = '🎥', label = 'Option', value = 'option', values = {
			{ label = "Start Recording", value = 'record', description = '' },
			{ label = "Save Recording", value = 'saveclip', description = '' },
			{ label = "Discard Recording", value = 'delclip', description = '' },
			{ label = "Open Editor", value = 'editor', description = '' }
		}})
...
RegisterNetEvent('ts-adminmenu:client:TruckPunchlinePly', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:TruckPunchlinePly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:ClownAttackPly', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:ClownAttackPly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:MerryAttackPly', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:MerryAttackPly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:FakeCallply', function(data)
	...
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:FakeCallply', isServer = true, args = {plyid = v.source, type = type}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:Lagply', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:LagGame', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:Flashbangply', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:FlashPly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:BlowPly', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:BlowPly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:EjectPly', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:EjectPly', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:Crashply', function()
	ESX.TriggerServerCallback('ts-adminmenu:server:GetOnlinePlayers', function(ply)
			...
			local count = 2
			for k,v in pairs(ply) do
					table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:Crashply', isServer = true, args = {plyid = v.source}}})
					count = count + 1
			end
			exports['zf_context']:openMenu(myMenu)
	end)
...
RegisterNetEvent('ts-adminmenu:client:PlayFart', function(plyList, type)
	...
	for k,v in pairs(players) do
		table.insert(myMenu, {id = count , header = v.name, txt = 'Player ID: '..v.source, params = {event = 'ts-adminmenu:server:PlayFartFromServer', isServer = true, args = {ply = v.source,type = fart}}})
		count = count + 1
	end
    exports['zf_context']:openMenu(myMenu)
end)
...
RegisterNetEvent('ts-adminmenu:client:RemoveItem', function(ply, list)
	...
	local items = list
		...
		for k,v in pairs(items) do
			if v then
			table.insert(myMenu, {id = count , header = v.label, txt = 'Remove '..v.name, params = {event = 'ts-adminmenu:client:RemoveItemCount', isServer = false, args = {item = v.name,plyid = pid}}})
			count = count + 1
			end
		end
		exports['zf_context']:openMenu(myMenu)
